...
const KomikPopuler = () => {
  ...
  useEffect(() => {
    setSeoMeta(
      'Komik Manga & Manhwa Terpopuler Hari Ini Sub Indo | Ndichan',
      'Daftar komik Manga, Manhwa, dan Manhua paling populer dan banyak dibaca hari ini subtitle Indonesia kualitas HD gratis tanpa iklan di Ndichan.',
      '/img/welcomebanner.webp',
      `${SITE_URL}/komik/populer`,
      {
        keywords: 'komik populer, manga populer, manhwa terpopuler, komik rating tertinggi, baca komik sub indo, ndichan',
        schema: {
          '@context': 'https://schema.org',
          '@graph': [
            getCollectionSchema({
              name: 'Komik Populer Hari Ini | Ndichan',
              description: 'Daftar komik terpopuler yang paling banyak dibaca di Ndichan.',
              url: `${SITE_URL}/komik/populer`
            }),
            getBreadcrumbSchema([
              { name: 'Home', url: '/' },
              { name: 'Komik', url: '/komik' },
              { name: 'Populer', url: '/komik/populer' }
            ])
          ]
        }
      }
  ...
  const fetchItems = useCallback(async (isInitial = false, cursorValue = null) => {
    ...
    try {
      const url = cursorValue
        ? `/ndikagantengtobrutbanget/v1/manga/populartoday?limit=${LIMIT}&after=${encodeURIComponent(cursorValue)}`
        : `/ndikagantengtobrutbanget/v1/manga/populartoday?limit=${LIMIT}`;
      ...
      <section className="max-w-7xl mx-auto px-4 md:px-6 pt-24">
        ...
        <div className="flex flex-col mb-8">
          ...
          <p className="text-xs md:text-sm text-white/50 pl-4 font-medium">
            Manga, Manhwa, dan Manhua paling banyak dibaca oleh komunitas NdiChan
          </p>
        </div>
        ...
        {!isLoading && error && (
          <div className="flex flex-col items-center justify-center py-24 text-center px-6">
            <p className="text-white/60 font-bold text-sm">Gagal memuat daftar komik populer.</p>
